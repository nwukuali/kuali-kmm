	  ...
	  <c:if test="${KualiForm.lookupCriteriaEnabled}">
		<tr>
			...
			<table align="center" cellpadding="0" cellspacing="0" class="datatable-100">
				...
				<c:set var="ActionName" value="reorderLookup.do" scope="request" />
			...
			<c:if test="${!empty reqSearchResultsActualSize }">
				...
				requestURI="reorderLookup.do?methodToCall=viewResults&reqSearchResultsActualSize=${reqSearchResultsActualSize}&searchResultKey=${searchResultKey}&searchUsingOnlyPrimaryKeyValues=${KualiForm.searchUsingOnlyPrimaryKeyValues}&actionUrlsExist=${KualiForm.actionUrlsExist}">
...
<%--NOTE: DO NOT FORMAT THIS FILE, DISPLAY:COLUMN WILL NOT WORK CORRECTLY IF IT CONTAINS LINE BREAKS --%>
						<c:otherwise>
							<display:column class="${colClass}" sortable="${column.sortable}"
								title="${column.columnTitle}" comparator="${column.comparator}"
								maxLength="${column.maxLength}" decorator="org.kuali.ext.mm.web.format.ReorderColumnFormatter"><c:out value="${column.propertyValue}" escapeXml="false" />&nbsp;</display:column>
                        </c:otherwise>
					</c:choose>
				</c:forEach>
