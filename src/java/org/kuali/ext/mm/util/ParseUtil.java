...
public class ParseUtil {
	...
	for (char c : dataChars) {
	    ...
	    } else if (c == WHITESPACE && lastTokenChar
		    && currIndex == dataChars.length - 1) {
		tokens.add("");
	    } else {
		lastTokenChar = false;
	    }
	    ...
	    if (c != delimiter && !withinQuote
		    && currIndex == data.length() - 1) {
		tokens.add(new String(dataChars, startIndex + 1,
			(currIndex - startIndex)).replace('\"', ' ').trim());
	    }
	    currIndex++;
	}
