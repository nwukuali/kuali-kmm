...
public class StockCountDaoJdbc extends PlatformAwareDaoBaseJdbc implements StockCountDao {
    ...
    public int dailyStockCountInsert() {
       // Clearing the Unassigned Count Records
        ...
        String sql = "insert into mm_stock_count_t (stock_count_id, obj_id, ver_nbr, stock_id, bin_id, worksheet_count_doc_nbr, "
                ...
                + "select mm_stock_count_s.nextval, sys_guid(), 1, sbal.stock_id, sbal.bin_id, null, ' ', "
                ...
                + "to_number(to_char(max(nvl(hist.history_trans_timestamp, sysdate - 366)), 'YYYYMMDD')) last_cnt_date, "
                + "to_number(to_char(max(nvl(cnt.last_update_dt, sysdate - 366)), 'YYYYMMDD')) last_wrksht_date from mm_stock_t stk "
                ...
                + "round(365 / cct.times_per_year_nbr)) stock where sbal.stock_id = stock.stkid and (stock.last_cnt_date + stock.days_btwn_cnt) "
                + "<= to_number(to_char(sysdate, 'YYYYMMDD')) and (stock.last_wrksht_date + stock.days_btwn_cnt) <= to_number(to_char(sysdate, 'YYYYMMDD')) "
                + "and not exists (select 1 from mm_stock_count_t unasnd where unasnd.stock_id = sbal.stock_id and unasnd.bin_id = sbal.bin_id "
    ...
    public int fullInventoryStockCountInsert(String warehouseCode, String zoneCode,
        ...
        String sql = "insert into mm_stock_count_t(stock_count_id, obj_id, ver_nbr, stock_id, bin_id, worksheet_count_doc_nbr, "
                ...
                + "and zn.warehouse_cd = '"
                + warehouseCode
                + "' and nvl(cct.times_per_year_nbr , 0) > 0 ";
