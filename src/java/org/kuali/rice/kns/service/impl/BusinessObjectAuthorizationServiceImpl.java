...
@Deprecated
//TODO: NWU - Evaluate kmm customization and remove this class if no longer required
		...
		for (MaintainableItemDefinition maintainableItemDefinition : itemDefinitions) {
			if (maintainableItemDefinition instanceof MaintainableCollectionDefinition) {
				try {
					...
					//TODO: NWU - KMM Customization starts
					Object prop = PropertyUtils.getProperty(businessObject,	maintainableItemDefinition.getName());
					if (prop != null && Collection.class.isAssignableFrom(prop.getClass())) {
						Collection<BusinessObject> collection = (Collection<BusinessObject>) prop;
						BusinessObjectEntry collectionBusinessObjectEntry = (BusinessObjectEntry) getDataDictionaryService()
								.getDataDictionary().getBusinessObjectEntry(maintainableCollectionDefinition.getBusinessObjectClass().getName());
					//TODO: NWU - KMM Customization ends
							int i = 0;
			     		for (Iterator<BusinessObject> iterator = collection.iterator(); iterator
							.hasNext();) {
