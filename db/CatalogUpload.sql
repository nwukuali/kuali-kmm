--prepare missing catalog groups based on unspc code
insert into mm_catalog_group_t
  (CATALOG_GROUP_CD,
   ...
   ACTV_IND,
   LAST_UPDATE_DT)
  select mm_catalog_group_s.nextval,
         sys_guid(),
         1,
         grps.grpnm,
         'Y',
    ...
    from (select distinct ltrim(rtrim(substr(a.family_title, 0, 45))) grpnm
           ...
           where a.code = b.unspsc_cd
             and b.catalog_pending_doc_nbr = '332029') grps
...
--add unique catalog subgroup names   based on unspc code
insert into mm_catalog_subgroup_t
  ...
  select MM_CATALOG_SUBGROUP_S.NEXTVAL,
         ...
         1,
         y.catalog_group_cd,
         MM_CATALOG_SUBGROUP_S.currval,
         subrps.grpdesc,
         'Y',
         current_date,
         null
    from (select distinct a.family_title grp,
                          ltrim(rtrim(substr(a.class_title, 0, 80))) grpdesc
            from mm_unspsc_t a, mm_catalog_item_pending_t b
           where a.code = b.unspsc_cd
             and b.catalog_pending_doc_nbr = '332029') subrps
...
--assign catalog code based on unspc code
update mm_catalog_item_pending_t a
   set a.catalog_group_cd   =
       (select distinct x.catalog_group_cd
          from mm_unspsc_t z
          join mm_catalog_group_t y
            on ltrim(rtrim(upper(substr(y.catalog_group_nm, 0, 45)))) =
         ...
         where z.code = a.unspsc_cd
           and rownum < 2),
       a.catalog_subgroup_cd =
       (select distinct x.catalog_subgroup_cd
          from mm_unspsc_t z
          join mm_catalog_group_t y
            on ltrim(rtrim(upper(substr(y.catalog_group_nm, 0, 45)))) =
               ltrim(rtrim(upper(substr(z.family_title, 0, 45))))
         ...
         where z.code = a.unspsc_cd
           and rownum < 2)
 where a.catalog_pending_doc_nbr = '332029'
   and a.catalog_group_cd is null;

--add missing unique catalog group names
insert into mm_catalog_group_t
  (CATALOG_GROUP_CD,
   OBJ_ID,
   VER_NBR,
  ...
  select '06' || mm_catalog_group_s.nextval,
    ...
    from (select distinct ltrim(rtrim(a.catalog_group_nm)) grpnm
           ...
           where a.catalog_pending_doc_nbr = '332029'
             and a.catalog_group_cd is null) grps
    left outer join mm_catalog_group_t x
      on x.catalog_group_nm = grps.grpnm
   where x.catalog_group_cd is null
     and grps.grpnm is not null;
...
insert into mm_catalog_subgroup_t
  ...
  select MM_CATALOG_SUBGROUP_S.NEXTVAL,
    ...
    from (select distinct a.catalog_group_cd grp,
            ...
            from mm_catalog_item_pending_t a
           where a.catalog_pending_doc_nbr = '332029'
             and a.catalog_subgroup_cd is null) subrps
    left outer join mm_catalog_subgroup_t x
   ...
   set x.MANUFACTURER_NBR        =
       (select MANUFACTURER_NBR
         ...
         where y.distributor_nbr = x.distributor_nbr
           and x.catalog_id = 5
           and y.catalog_pending_doc_nbr = '332029'),
 ...
 where x.catalog_item_id in
       (select a.catalog_item_id
         ...
         where a.distributor_nbr = b.distributor_nbr
           and a.catalog_id = 5
           and b.catalog_pending_doc_nbr = '332029');
...
update mm_catalog_item_image_t d
   set d.CATALOG_IMAGE_ID =
       (select CATALOG_IMAGE_ID
         ...
         where d.catalog_item_id = b.catalog_item_id
           ...
           and a.catalog_pending_doc_nbr = '332029')
 where d.catalog_item_image_id in
       (select catalog_item_image_id
         ...
         where x.catalog_pending_doc_nbr = '332029');
