select extract(MONTH from orderDoc.Order_Create_Dt) as "Period Date (Month)",
       ...
       (select SUM((odtl.order_item_qty * odtl.Order_Item_Price_Amt) +
          ...
          from ${namespace}.MM_ORDER_DOC_T odoc
          left join ${namespace}.MM_ORDER_DETAIL_T odtl
         ...
         where odoc.profile_typ_cd = 'INTERNAL'
           ...
           and odoc.order_create_dt between to_date('${fromDate}', 'MM/DD/YYYY') and
               to_date('${toDate}', 'MM/DD/YYYY')) as "Department Total",
  ...
  left join ${namespace}.MM_PROFILE_T custProfile
    on custProfile.Profile_Id = orderDoc.Customer_Profile_Id
  left join ${namespace}.MM_SALES_INSTANCE_T salesInstance
    on salesInstance.Order_Doc_Nbr = orderDoc.Fdoc_Nbr
  left join ${namespace}.MM_ACCOUNTS_T accnt
    on accnt.order_detail_id = detail.order_detail_id
  where orderDoc.order_create_dt between to_date('${fromDate}', 'MM/DD/YYYY') 
