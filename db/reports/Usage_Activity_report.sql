select catalog.catalog_begin_dt as "Period Date (start)",
       ...
       nvl((select SUM(d.order_item_qty) / MONTHS_BETWEEN(to_date('01/01/2011', 'MM/DD/YYYY'), to_date('01/01/2010', 'MM/DD/YYYY'))
       ...
       nvl((select MAX(sc.stock_cst)
         ...
         where sc.stock_id = item.stock_id
           and sc.cost_cd = '02') * (select SUM(d.order_item_qty) / MONTHS_BETWEEN(to_date('01/01/2011', 'MM/DD/YYYY'), to_date('01/01/2010', 'MM/DD/YYYY'))
          from MM_ORDER_DETAIL_T d
          left join MM_ORDER_DOC_T o
            on o.fdoc_nbr = d.order_doc_nbr
         where o.order_create_dt between to_date('01/01/2010', 'MM/DD/YYYY') and
